...
export default {
  data() {
    return {
      id: this.$route.params.id, //将路由参数id挂载到data上 方便后期调用
      lunbotu: [], //轮播图数据
      goodsinfo: {}, //获取到的商品的信息
      ballFlag: false,
  ...
  methods: {
    ...
    dropping(el, done) {
      ...
      const badgePosition = document.getElementById('badge').getBoundingClientRect(); //获取最终位置
